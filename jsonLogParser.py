...
def user_identity_parser(data_base_connection, entity_name, entity_type, arn, region):
    if not node_exist(data_base_connection, entity_name):
        return
    # entity_type = "aws service"
    # entity_node = Node(entity_type,name=entity_name,{arn=str(arn),region=str(region)})
    query_enter_identity = f"""
    CREATE (n:{str(entity_type)} {{name: '{str(entity_name)}', arn: '{str(arn)}, region: {str(region)}'}})
    """
...
def link_parser(data_base_connection, entity_name, entity_source_name, event_type, timestamp, source_ip, user_agent):
    ...
    WHERE a.name = '{entity_name}' AND b.name = '{entity_source_name}'
